...
# Configurar página 
st.set_page_config(page_title="Malla Curricular LQFB", page_icon="🎓", layout="wide")

tab1, tab2, tab3 = st.tabs([" Normal", " Recomendado", " Personalizado"])
altura = 1290
html_grid = importlib.reload(html_grid)

with tab1:
    # Embeber HTML
    components.html(html_grid.HTML_LQFB_N, height=altura, scrolling=True)
with tab2:
    # Embeber HTML
    components.html(html_grid.HTML_LQFB_M, height=altura, scrolling=True)
with tab3:
    # Embeber HTML
    components.html(html_grid.HTML_LQFB_P, height=altura, scrolling=True)
